...
const profController = {
    ...
    getMessages: (req, res) => {
        const db = getDb();
        db.all(`SELECT m.*, u.nom as exp_nom, u.prenom as exp_prenom FROM messages m LEFT JOIN users u ON m.expediteur_id = u.id WHERE m.destinataire_id = ? OR m.destinataire_role = 'all' OR m.destinataire_role = 'prof' ORDER BY m.created_at DESC LIMIT 30`,
