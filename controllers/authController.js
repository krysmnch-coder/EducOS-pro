...
const authController = {
    register: (req, res) => {
        ...
        db.get('SELECT allow_registration FROM settings WHERE id=1', [], (err, row) => {
            if (row && row.allow_registration == 0) {
                return res.redirect('/auth/register?error=Inscriptions désactivées par l\'administration');
            }
            ...
            db.get('SELECT max_users FROM settings WHERE id=1', [], (err, setting) => {
                const maxUsers = setting ? setting.max_users : 500;
                db.get('SELECT COUNT(*) as total FROM users', [], (err, count) => {
                    if (count && count.total >= maxUsers) {
                        return res.redirect('/auth/register?error=Nombre maximum d\'utilisateurs atteint (' + maxUsers + ')');
                    }
                    ...
                    db.get('SELECT id FROM users WHERE email = ?', [email], (err, user) => {
                        ...
                        bcrypt.hash(password, 10, (err, hashedPassword) => {
                            ...
                            // Rôle par défaut depuis les paramètres
                            db.get('SELECT default_role FROM settings WHERE id=1', [], (err, setting) => {
                                const finalRole = role || (setting ? setting.default_role : 'eleve');
                                ...
                                if (finalRole === 'parent') {
                                    ...
                                    db.run('INSERT INTO users (nom, prenom, email, password, role, classes_assignees) VALUES (?, ?, ?, ?, ?, ?)',
                                        [nom, prenom, email, hashedPassword, finalRole, JSON.stringify(enfants)], function(err) {
    ...
    login: (req, res) => {
        ...
        db.get('SELECT * FROM users WHERE email = ? AND compte_actif = 1', [email], (err, user) => {
            ...
            bcrypt.compare(password, user.password, (err, isMatch) => {
                ...
                req.session.user = { id: user.id, email: user.email, nom: user.nom, prenom: user.prenom, role: user.role, photo: user.photo };
