   ...
   register: (req, res) => {
    ...
    db.get('SELECT id FROM users WHERE email = ?', [email], (err, user) => {
        ...
        bcrypt.hash(password, 10, (err, hashedPassword) => {
            ...
            // Pour le parent : stocker les enfants
            if (role === 'parent') {
                ...
                for (let i = 0; i < enfantsNoms.length; i++) {
                    if (enfantsNoms[i] && enfantsPrenoms[i]) {
                        enfants.push({
                            nom: enfantsNoms[i],
                            prenom: enfantsPrenoms[i],
                            classe: enfantsClasses[i] || '',
                            source: 'declaration_parent'
                        });
                    }
                }
                ...
                db.run('INSERT INTO users (nom, prenom, email, password, role, classes_assignees) VALUES (?, ?, ?, ?, ?, ?)',
                    [nom, prenom, email, hashedPassword, role, JSON.stringify(enfants)], function(err) {
                        if (err) return res.redirect('/auth/register?error=Erreur inscription parent');
                        
                        // Créer automatiquement les comptes enfants s'ils n'existent pas
                        enfants.forEach(enfant => {
                            db.get("SELECT id FROM users WHERE nom = ? AND prenom = ? AND role = 'eleve'", 
                                [enfant.nom, enfant.prenom], (err, row) => {
                                if (!row) {
                                    // Créer un compte élève basique
                                    bcrypt.hash('educos2024', 10, (err, hash) => {
                                        db.run("INSERT INTO users (nom, prenom, email, password, role, classes_assignees, date_naissance) VALUES (?, ?, ?, ?, 'eleve', ?, ?)",
                                            [enfant.nom, enfant.prenom, 
                                             enfant.prenom.toLowerCase() + '.' + enfant.nom.toLowerCase() + '@eleve.educos.com',
                                             hash, enfant.classe || '', date_naissance || '']);
                                    });
                                }
                            });
                        });
                        
                        res.redirect('/auth/login?success=Compte parent créé ! Vos enfants ont été enregistrés.');
                    });
            }
            // Pour l'élève : stocker sa classe
            else if (role === 'eleve') {
                db.run('INSERT INTO users (nom, prenom, email, password, role, classes_assignees, date_naissance) VALUES (?, ?, ?, ?, ?, ?, ?)',
                    [nom, prenom, email, hashedPassword, role, classe_eleve, date_naissance], function(err) {
                        if (err) return res.redirect('/auth/register?error=Erreur inscription élève');
                        res.redirect('/auth/login?success=Compte élève créé ! Connectez-vous.');
                    });
            }
            // Pour le professeur
            else if (role === 'prof') {
                db.run('INSERT INTO users (nom, prenom, email, password, role, matiere_principale, classes_assignees) VALUES (?, ?, ?, ?, ?, ?, ?)',
                    [nom, prenom, email, hashedPassword, role, matiere_principale, classes_assignees], function(err) {
                        if (err) return res.redirect('/auth/register?error=Erreur inscription professeur');
    ...
    login: (req, res) => {
        ...
        if (!email || !password) {
            return res.redirect('/auth/login?error=Email et mot de passe requis');
        }
