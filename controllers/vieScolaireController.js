...
const vsController = {
    getStats: (req, res) => {
        ...
        db.get("SELECT COUNT(*) as total FROM absences WHERE date_absence = ? AND type = 'absence'", [today], (err, row) => {
            ...
            db.get("SELECT COUNT(*) as total FROM absences WHERE date_absence = ? AND type = 'retard'", [today], (err, row) => {
                ...
                db.get("SELECT COUNT(*) as total FROM absences WHERE date_absence >= ?", [debutMois], (err, row) => {
                    stats.absencesMois = row?.total || 0;
                    db.get("SELECT COUNT(*) as total FROM pointage WHERE date_pointage = ? AND (statut = 'present' OR statut = 'sortie')", [today], (err, row) => {
                        ...
                        db.get("SELECT COUNT(*) as total FROM pointage WHERE date_pointage = ? AND statut = 'absent'", [today], (err, row) => {
                            stats.profsAbsents = row?.total || 0;
    ...
    getAbsences: (req, res) => {
        ...
        const type = req.query.type || '', search = req.query.search || '', justifie = req.query.justifie || '';
        let where = [], params = [];
        if (type) { where.push('a.type = ?'); params.push(type); }
        if (search) { where.push('(u.nom LIKE ? OR u.prenom LIKE ?)'); params.push(`%${search}%`, `%${search}%`); }
        if (justifie !== '') { where.push('a.justifie = ?'); params.push(justifie); }
    ...
    checkAvertissement: (profId) => {
        ...
        db.get('SELECT SUM(COALESCE(minutes_retard, 0)) as total FROM pointage WHERE prof_id = ? AND strftime("%Y-%m", date_pointage) = ?',
            [profId, mois], (err, row) => {
            if (err) return;
    ...
    getProfs: (req, res) => {
        globalDb.all("SELECT id, nom, prenom, civilite, telephone, matiere_principale FROM users WHERE role = 'prof' AND compte_actif = 1 ORDER BY nom", [], (err, rows) => res.json(rows || []));
    },

    getEleves: (req, res) => {
        globalDb.all("SELECT id, nom, prenom, email, telephone, classes_assignees FROM users WHERE role = 'eleve' AND compte_actif = 1 ORDER BY nom", [], (err, rows) => res.json(rows || []));
    },
    ...
    getUsersByRole: (req, res) => {
        ...
        globalDb.all(`SELECT id, nom, prenom, email, telephone, role FROM users WHERE ${where.join(' AND ')} ORDER BY nom`, params, (err, rows) => res.json(rows || []));
    },

    updateUserTelephone: (req, res) => {
        globalDb.run('UPDATE users SET telephone = ? WHERE id = ?', [req.body.telephone, req.params.id], (err) => {
            if (err) return res.status(500).json({ error: 'Erreur' });
