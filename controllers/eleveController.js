...
const eleveController = {
    ...
    getEDT: (req, res) => {
        globalDb.get('SELECT classes_assignees as classe FROM users WHERE id = ?', [req.session.user.id], (err, row) => {
...
viderNotifications: (req, res) => {
    ...
    rechercherEleves: (req, res) => {
        ...
        globalDb.all("SELECT id, nom, prenom, classes_assignees as classe FROM users WHERE role = 'eleve' AND compte_actif = 1 AND etablissement_code = ? AND id != ? AND (nom LIKE ? OR prenom LIKE ?) ORDER BY nom LIMIT 20", 
            [etablissementCode, userId, '%' + search + '%', '%' + search + '%'], (err, rows) => res.json(rows || []));
    },
