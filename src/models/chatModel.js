...
const getMessages = async (userId1, userId2) => {
    try {
        console.log('🔍 chatModel.getMessages appelé avec:', userId1, 'et', userId2);
        ...
        if (!conversation || conversation.length === 0) {
            console.log('⚠️ Aucune conversation trouvée, retour tableau vide');
            return [];
        }
        ...
        console.error('❌ Erreur dans chatModel.getMessages:', error);
        throw error;
    }
};
...
const getOrCreateConversation = async (userId1, userId2) => {
    try {
        ...
        if (conversation && conversation.length > 0) {
            console.log('✅ Conversation trouvée, ID:', conversation[0].id);
