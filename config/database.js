...
// Base GLOBALE (admin, établissements, connexions)
const globalDbPath = path.join(dbDir, 'educos_global.db');
const globalDb = new sqlite3.Database(globalDbPath, (err) => {
    if (err) console.error('❌ Erreur base globale:', err.message);
    else { console.log('✅ Base globale connectée'); createGlobalTables(); }
});

// Base ÉTABLISSEMENT (dynamic)
let etablissementDb = null;

function getEtablissementDb() { return etablissementDb; }

function setEtablissementDb(dbName) {
    if (etablissementDb) { try { etablissementDb.close(); } catch(e) {} }
    const dbPath = path.join(dbDir, dbName);
    etablissementDb = new sqlite3.Database(dbPath, (err) => {
        if (err) console.error('❌ Erreur base établissement:', err.message);
        else { console.log('✅ Base établissement connectée:', dbName); createEtablissementTables(); }
    });
...
function createGlobalTables() {
    globalDb.serialize(() => {
        globalDb.run(`CREATE TABLE IF NOT EXISTS etablissements (
            ...
            directeur TEXT DEFAULT '', annee_scolaire TEXT DEFAULT '2024-2025',
            db_name TEXT NOT NULL, actif INTEGER DEFAULT 1,
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP
        )`);
        globalDb.run(`CREATE TABLE IF NOT EXISTS admins (
            id INTEGER PRIMARY KEY AUTOINCREMENT, email TEXT UNIQUE NOT NULL, password TEXT NOT NULL,
            nom TEXT NOT NULL, prenom TEXT NOT NULL, etablissement_code TEXT NOT NULL,
            compte_actif INTEGER DEFAULT 1, created_at DATETIME DEFAULT CURRENT_TIMESTAMP
        )`);
...
function createEtablissementTables() {
    if (!etablissementDb) return;
    etablissementDb.serialize(() => {
        etablissementDb.run(`CREATE TABLE IF NOT EXISTS users (
            id INTEGER PRIMARY KEY AUTOINCREMENT, email TEXT UNIQUE NOT NULL, password TEXT NOT NULL,
            role TEXT CHECK(role IN ('vie_scolaire', 'prof', 'parent', 'eleve')) NOT NULL,
            nom TEXT NOT NULL, prenom TEXT NOT NULL, civilite TEXT DEFAULT 'M.',
            date_naissance DATE, telephone TEXT, adresse TEXT,
            matiere_principale TEXT, classes_assignees TEXT, photo TEXT DEFAULT 'default.png',
            compte_actif INTEGER DEFAULT 1, derniere_connexion DATETIME,
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP
        )`);
        etablissementDb.run(`CREATE TABLE IF NOT EXISTS settings (id INTEGER PRIMARY KEY DEFAULT 1, max_users INTEGER DEFAULT 500, default_role TEXT DEFAULT 'eleve', maintenance_mode INTEGER DEFAULT 0, allow_registration INTEGER DEFAULT 1, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`);
        etablissementDb.run(`CREATE TABLE IF NOT EXISTS absences (id INTEGER PRIMARY KEY AUTOINCREMENT, eleve_id INTEGER NOT NULL, date_absence DATE NOT NULL, type TEXT CHECK(type IN ('absence','retard')) NOT NULL, motif TEXT DEFAULT 'Non justifié', justifie INTEGER DEFAULT 0, duree_minutes INTEGER DEFAULT 0, signale_par TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`);
        ...
        etablissementDb.run(`CREATE TABLE IF NOT EXISTS paiements (id INTEGER PRIMARY KEY AUTOINCREMENT, type TEXT NOT NULL, categorie TEXT NOT NULL, montant INTEGER NOT NULL, description TEXT, date_paiement DATE DEFAULT (date('now')), beneficiaire TEXT, mode_paiement TEXT DEFAULT 'especes', reference TEXT, user_id INTEGER, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`);
        ...
        etablissementDb.run(`CREATE TABLE IF NOT EXISTS devoirs_rendus (id INTEGER PRIMARY KEY AUTOINCREMENT, ressource_id INTEGER NOT NULL, eleve_id INTEGER NOT NULL, fichier TEXT, note REAL, rendu_le DATETIME DEFAULT CURRENT_TIMESTAMP)`);
        etablissementDb.run(`CREATE TABLE IF NOT EXISTS notes (id INTEGER PRIMARY KEY AUTOINCREMENT, eleve_id INTEGER NOT NULL, matiere TEXT NOT NULL, classe TEXT NOT NULL, type_evaluation TEXT NOT NULL, note REAL NOT NULL, coefficient INTEGER DEFAULT 1, trimestre TEXT, prof_id INTEGER, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`);
        ...
        etablissementDb.run(`CREATE TABLE IF NOT EXISTS groupes (id INTEGER PRIMARY KEY AUTOINCREMENT, nom TEXT NOT NULL, createur_id INTEGER NOT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`);
        ...
        etablissementDb.run(`CREATE TABLE IF NOT EXISTS messages_groupes (id INTEGER PRIMARY KEY AUTOINCREMENT, groupe_id INTEGER NOT NULL, expediteur_id INTEGER NOT NULL, contenu TEXT NOT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`);
        etablissementDb.run(`CREATE TABLE IF NOT EXISTS messages_amis (id INTEGER PRIMARY KEY AUTOINCREMENT, expediteur_id INTEGER NOT NULL, destinataire_id INTEGER NOT NULL, contenu TEXT NOT NULL, lu INTEGER DEFAULT 0, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`);
        etablissementDb.run(`CREATE TABLE IF NOT EXISTS fiches_eleves (id INTEGER PRIMARY KEY AUTOINCREMENT, parent_id INTEGER, nom TEXT NOT NULL, prenom TEXT NOT NULL, date_naissance TEXT, classe_actuelle TEXT, pere_nom TEXT, pere_telephone TEXT, mere_nom TEXT, mere_telephone TEXT, allergie INTEGER DEFAULT 0, asthme INTEGER DEFAULT 0, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`);
